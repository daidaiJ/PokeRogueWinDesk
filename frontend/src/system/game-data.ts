...
export class GameData {
  ...
  public saveSystem(): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      ...
      localStorage.setItem(`data_panda`, encrypt(systemData, bypassLogin));
  ...
  public loadSystem(): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      ...
      if (bypassLogin && !localStorage.getItem(`data_panda`)) {
        return resolve(false);
      }
      ...
      if (!bypassLogin) {
        pokerogueApi.savedata.system.get({ clientSessionId })
          .then(saveDataOrErr => {
            ...
            const cachedSystem = localStorage.getItem(`data_panda`);
            this.initSystem(saveDataOrErr, cachedSystem ? AES.decrypt(cachedSystem, saveKey).toString(enc.Utf8) : undefined).then(resolve);
          });
      } else {
        this.initSystem(decrypt(localStorage.getItem(`data_panda`)!, bypassLogin)).then(resolve); // TODO: is this bang correct?
      }
    });
  }
  ...
  public initSystem(systemDataStr: string, cachedSystemDataStr?: string): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      try {
        ...
        localStorage.setItem(`data_panda`, encrypt(systemDataStr, bypassLogin));

        const lsItemKey = `runHistoryData_panda`;
      ...
      */
       ...
       *
        ...
        */
        ...
        localStorage.setItem(`runHistoryData_panda`, "");
        return {};
      }
    } else {
      const lsItemKey = `runHistoryData_panda`;
  ...
  async saveRunHistory(scene: BattleScene, runEntry: SessionSaveData, isVictory: boolean): Promise<boolean> {
    ...
    localStorage.setItem(`runHistoryData_panda`, encrypt(JSON.stringify(runHistoryData), bypassLogin));
    /**
     * Networking Code DO NOT DELETE
     *
    if (!Utils.isLocal) {
  ...
  public clearLocalData(): void {
    ...
    localStorage.removeItem(`data_panda`);
    for (let s = 0; s < 5; s++) {
  ...
  saveAll(scene: BattleScene, skipVerification: boolean = false, sync: boolean = false, useCachedSession: boolean = false, useCachedSystem: boolean = false): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      Utils.executeIf(!skipVerification, updateUserInfo).then(success => {
        ...
        const maxIntAttrValue = 0x80000000;
        const systemData = useCachedSystem ? this.parseSystemData(decrypt(localStorage.getItem(`data_panda`)!, bypassLogin)) : this.getSystemSaveData(); // TODO: is this bang correct?
        ...
        localStorage.setItem(`data_panda`, encrypt(JSON.stringify(systemData, (k: any, v: any) => typeof v === "bigint" ? v <= maxIntAttrValue ? Number(v) : v.toString() : v), bypassLogin));
